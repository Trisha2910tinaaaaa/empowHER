...
export function JobChatbot({ fullScreen = false, modernUI = false }: JobChatbotProps) {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'assistant',
      content: 'Hi there! I can help you with job search or answer any questions about careers in tech. What would you like to know?',
      timestamp: new Date(),
    },
  ]);
  ...
  const loadSessions = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/chat/sessions?active_only=true${userParam}`, {
        headers
      });
  ...
  const deleteSession = async (sessionId: string) => {
    try {
      ...
      await fetch(`http://localhost:8000/api/chat/sessions/${sessionId}?delete_messages=true`, {
        method: 'DELETE',
        headers
      });
                          ...
                          >
                            <div className="flex items-center gap-2 overflow-hidden flex-1">
                              ...
                              <div className="overflow-hidden">
                                <p className="text-sm font-medium truncate text-gray-800 dark:text-gray-200">{session.title}</p>
